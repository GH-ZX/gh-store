...
export function HomepageClient({
  ...
  const featuredProducts = featuredData && featuredData.length > 0 ? featuredData : initialFeatured;
  ...
  const globalList = useMemo(() => allProducts ?? [], [allProducts]);
  ...
  return (
    <main className="min-h-screen">
      ...
      {carouselProducts.length > 0 && (
        <section className="mx-auto max-w-7xl px-4 pt-8 pb-6 md:pt-12 md:pb-8">
          ...
          <div className="mt-4 sm:mt-6">
            <QuickActions
              games={gameProducts.length > 0 ? globalList.filter((p) => p.type === "topup") : []}
              isRtl={isRtl}
            />
          </div>
        </section>
      ...
      {voucherCount > 0 && (
        <section className="py-10 md:py-14">
          <div className="mx-auto max-w-7xl px-4">
            <GiftCardScroller
              vouchers={globalList.filter((p) => p.type !== "topup")}
              isRtl={isRtl}
            />
          </div>
        </section>
          ...
          <div className="mx-auto max-w-7xl space-y-6 px-4">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
              {gameProducts.map((game) => {
                ...
                const catalogue =
                  (gameMeta?.catalogue as Array<{ name: string; amount: number }> | undefined) ??
                  [];
                const sorted = [...catalogue].sort((a, b) => a.amount - b.amount);
                const minPrice = sorted.length > 0 ? sorted[0].amount : game.basePrice;
                const maxPrice =
                  sorted.length > 0 ? sorted[sorted.length - 1].amount : game.basePrice;

                return (
                  <Link
                    key={game.id}
                    href={`/${locale}/store/${game.slug}`}
                    className="group bg-card relative overflow-hidden rounded-xl border transition-all hover:-translate-y-0.5 hover:shadow-lg"
                  >
                    {/* Game image */}
                    <div className="bg-muted/30 relative aspect-[4/3] overflow-hidden">
                      {game.imageUrl ? (
                        ...
                        <div className="flex h-full w-full items-center justify-center">
                          <Gamepad2 className="text-muted-foreground/20 size-12" />
                        </div>
                      )}
                      {/* Price badge */}
                      <div className="absolute top-2 right-2">
                        <Badge
                          variant="secondary"
                          className="bg-background/80 text-xs font-semibold backdrop-blur-sm"
                        >
      ...
      <section className="bg-background border-t py-16 md:py-20">
        <div className="mx-auto max-w-7xl px-4">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {features.map((feature, index) => (
              <div
                key={index}
                className="group bg-card rounded-xl border p-6 transition-all hover:-translate-y-0.5 hover:shadow-md"
              >
...
const features = [
  {
    ...
    descAr: "احصل على منتجاتك الرقمية فورًا بعد الدفع",
    descEn: "Receive your digital products instantly after payment",
  },
  {
    icon: "🔒",
    titleAr: "دفع آمن",
    titleEn: "Secure Payment",
    descAr: "بوابات دفع موثوقة ومشفرة بالكامل",
    descEn: "Trusted, fully encrypted payment gateways",
  },
  {
    icon: "🎮",
    titleAr: "آلاف المنتجات",
    titleEn: "Thousands of Products",
    descAr: "أكبر تشكيلة من التوب أب والبطاقات الرقمية",
    descEn: "The largest selection of top-ups and digital cards",
  },
  {
    icon: "💬",
    titleAr: "دعم متواصل",
    titleEn: "24/7 Support",
    descAr: "فريق دعم جاهز لمساعدتك في أي وقت",
    descEn: "Support team ready to help you anytime",
  },
  {
    icon: "🏷️",
    titleAr: "أفضل الأسعار",
    titleEn: "Best Prices",
    descAr: "أسعار تنافسية مع تحديث يومي",
    descEn: "Competitive prices updated daily",
  },
  {
    icon: "🌐",
    titleAr: "منتجات عالمية",
    titleEn: "Global Products",
    descAr: "منتجات رقمية من جميع أنحاء العالم",
    descEn: "Digital products from around the world",
  },
];
