 ...
 */
async function resolveSamProviderId(serviceClient: ReturnType<typeof createClient>): Promise<string> {
  ...
  const { data: inserted, error } = await serviceClient
    .from('providers')
    .insert({ name: 'SAM API', slug: 'sam-api', description: 'ShamCash & Syriatel Cash payment gateway', is_active: true })
    .select('id')
    .single();
...
async function resolveSamApiKey(serviceClient: ReturnType<typeof createClient>, providerId: string) {
...
async function resolveWebhookSecret(serviceClient: ReturnType<typeof createClient>, providerId: string) {
...
Deno.serve(async (req) => {
  ...
  if (action === 'saveSettings') {
    ...
    if (p_regenerate_webhook) {
      ...
      await serviceClient
        .from('provider_credentials')
        .insert({ provider_id: samProviderId, key: 'webhook_secret', value: newSecret, is_active: true });
    }
  ...
  if (action === 'createInvoice') {
    ...
    if (!method || !identifier || !amount) {
      return jsonResponse({ success: false, message: 'method, identifier, and amount are required' }, 400);
    }
    ...
    const webhookUrl = webhookSecret
      ? buildWebhookUrl(supabaseUrl, webhookSecret)
      : '';
    ...
    if (!res.ok) {
      return jsonResponse({ success: false, message: samErrorMessage(data as Json, 'Failed to create invoice') }, res.status);
    }
